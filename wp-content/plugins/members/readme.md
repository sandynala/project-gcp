...
* **Explkwsly Deny Capabilities:** Deny specific caps to specific user roles.
...
* **Denying** a capability means that the role's users are explkwsly denied permission.
...
Instead, you could explkwsly *deny* the publish posts capability to the **Super** role.  When you do that, **User A** is only *granted* the following capabilities:
