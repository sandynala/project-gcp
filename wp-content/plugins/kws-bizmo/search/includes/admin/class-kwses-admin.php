...
class KWSES_Admin {
	...
	public function __construct() {
		...
		add_filter( 'plugin_action_links_' . KWSES_FILENAME, array( $this, 'plugin_action_links' ) );
	}
	...
	public function add_setting_pages() {
		...
		add_menu_page( 'Search Settings', 'Search', 'manage_options', 'wp-es', null, $this->get_menu_icon() );
		add_submenu_page( 'wp-es', 'Search Settings', 'Search Settings', 'manage_options', 'wp-es', array( $this, 'wp_es_page' ) );

		// Shift main setting page to top.
		if ( current_user_can( 'manage_options' ) ) {
			$kwses_menu = $submenu['wp-es'];
			$cpt_items = array_shift( $kwses_menu );
			array_push( $kwses_menu, $cpt_items );
			$submenu['wp-es'] = $kwses_menu;
		}
	}
	...
	public function wp_es_page(){ ?>
		<div class="wrap">
			<h2>Search <?php _e( 'Settings', 'wp-extended-search' ); ?></h2>
			<?php settings_errors(); ?>
			<form method="post" action="options.php">
				...
				if ( empty( KWSES()->kwses_settings['disabled'] ) ) {
					submit_button( __( 'Save Changes' ), 'primary', 'submit', false );
					echo '&nbsp;&nbsp;';
					submit_button( __( 'Reset to WP default', 'wp-extended-search' ), 'secondary', 'reset', false );
					if ( empty( KWSES()->current_setting_id ) ) {
						echo '&nbsp;&nbsp;';
						submit_button( __( 'Disable KWSES for global search', 'wp-extended-search' ), 'secondary', 'disable_global', false );
					}
				} else {
					submit_button( __( 'Enable KWSES for global search', 'wp-extended-search' ), 'primary', 'enable_global', false );
				}
				?>
			</form>
		</div>
	...
	public function admin_init() {
		// Register Settings.
		register_setting( 'wp_es_option_group', KWSES()->option_key_name, array( $this, 'wp_es_save' ) );
		...
		if ( empty( KWSES()->kwses_settings['disabled'] ) ) {
			...
			add_settings_section( 'wp_es_section_disabled', __( 'KWSES is disabled for global WordPress search. Select setting name to manage other search settings.', 'wp-extended-search' ), null, 'wp-es' );
		}
		...
		if ( ! KWSES()->kwses_settings['wc_search'] ) {
	...
	public function register_scripts( $hook ) {
		...
		if ( 'toplevel_page_wp-es' === $hook ) {
			...
			wp_enqueue_script( 'kwses_select2_js', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js' );
			wp_enqueue_script( 'kwses_admin_js', KWSES_ASSETS_URL . 'js/wp-es-admin.js', array( 'jquery-ui-datepicker', 'kwses_select2_js' ) );
			wp_enqueue_style( 'kwses_jquery_ui', KWSES_ASSETS_URL . 'css/jQueryUI/jquery-ui.min.css' );
			wp_enqueue_style( 'kwses_jquery_ui_theme', KWSES_ASSETS_URL . 'css/jQueryUI/jquery-ui.theme.min.css' );
			wp_enqueue_style( 'kwses_select2_css', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css' );
			wp_enqueue_style( 'kwses_admin_css', KWSES_ASSETS_URL . 'css/wp-es-admin.css', array( 'kwses_jquery_ui', 'kwses_jquery_ui_theme', 'kwses_select2_css' ) );
			...
			wp_localize_script(
				...
				array(
					...
					'new_setting_url'         => admin_url( 'post-new.php?post_type=kwses_setting' ),
		...
		} elseif ( get_current_screen()->id === 'kwses_setting' ) {
			wp_enqueue_style( 'kwses_admin_css', KWSES_ASSETS_URL . 'css/wp-es-admin.css' );
			wp_enqueue_script( 'kwses_admin_cpt_js', KWSES_ASSETS_URL . 'js/wp-es-setting-cpt.js' );

			wp_localize_script(
				'kwses_admin_cpt_js',
				'kwses_admin_cpt_vars',
				array(
	...
	public function wp_es_fields() {
		 ...
		 */
		$wp_es_fields = $wpdb->get_results( apply_filters( 'kwses_meta_keys_query', "select DISTINCT meta_key from $wpdb->postmeta where meta_key NOT LIKE '\_%' ORDER BY meta_key ASC" ) );
		$meta_keys    = array();
		 ...
		 */
		return apply_filters( 'kwses_meta_keys', $meta_keys );
	}
	...
	public function wp_es_save( $input ) {
		$settings = KWSES()->kwses_settings;

		if ( isset( $_POST['disable_global'] ) ) {
			$input['disabled'] = true;
			add_settings_error( 'wp_es_error', 'wp_es_error_disable_global', __( 'KWSES for global search has been disabled.', 'wp-extended-search' ), 'updated' );
			return $input;
		}

		if ( isset( $_POST['enable_global'] ) ) {
			$settings['disabled'] = false;
			add_settings_error( 'wp_es_error', 'wp_es_error_enable_global', __( 'KWSES for global search has been enabled.', 'wp-extended-search' ), 'updated' );
			return $settings;
		}
	...
	public function wp_es_section_content() {
		?>
		<em><?php _e( 'Every field have OR relation with each other. e.g. if someone search for "kwstech" then search results will show those items which have "kwstech" as meta value or taxonomy\'s term or in title or in content, whatever option is selected.', 'wp-extended-search' ); ?></em>
		<?php
	}
	...
	public function wp_es_settings_name() {
		...
		?>
		<select id="kwsessid" name="kwsessid">
			<option value=""><?php _e( 'Global (default)', 'wp-extended-search' ); ?></option>
			<?php
			foreach ( $all_settings as $setting_name ) {
				?>
			<option <?php selected( $setting_name->ID, KWSES()->current_setting_id ); ?> value="<?php echo $setting_name->ID; ?>"><?php echo esc_attr( get_the_title( $setting_name ) ); ?></option>
				<?php
			}
			?>
		...
		if ( ! empty( KWSES()->current_setting_id ) ) {
			echo '&nbsp;<a href="' . get_edit_post_link( KWSES()->current_setting_id ) . '">' . __( 'Edit Name', 'wp-extended-search' ) . '</a>';
		}
	}
	...
	public function wp_es_wc_search() {
		?>
		<input type="hidden" name="<?php echo KWSES()->option_key_name; ?>[wc_search]" value="0" />
		<input <?php checked( KWSES()->kwses_settings['wc_search'] ); ?> type="checkbox" id="es_wc_search" name="<?php echo KWSES()->option_key_name; ?>[wc_search]" value="1" />&nbsp;
	...
	public function wp_es_title_content_checkbox() {
		?>
		<input type="hidden" name="<?php echo KWSES()->option_key_name; ?>[title]" value="0" />
		<input <?php checked( KWSES()->kwses_settings['title'] ); ?> type="checkbox" id="estitle" name="<?php echo KWSES()->option_key_name; ?>[title]" value="1" />&nbsp;
		<label for="estitle"><?php _e( 'Search in Title', 'wp-extended-search' ); ?></label>
		<br />
		<input type="hidden" name="<?php echo KWSES()->option_key_name; ?>[content]" value="0" />
		<input <?php checked( KWSES()->kwses_settings['content'] ); ?> type="checkbox" id="escontent" name="<?php echo KWSES()->option_key_name; ?>[content]" value="1" />&nbsp;
		<label for="escontent"><?php _e( 'Search in Content', 'wp-extended-search' ); ?></label>
		<br />
		<input type="hidden" name="<?php echo KWSES()->option_key_name; ?>[excerpt]" value="0" />
		<input <?php checked( KWSES()->kwses_settings['excerpt'] ); ?> type="checkbox" id="esexcerpt" name="<?php echo KWSES()->option_key_name; ?>[excerpt]" value="1" />&nbsp;
	...
	public function wp_es_custom_field_name_list() {
		...
		if ( ! empty( $meta_keys ) ) {
			?>
			<select class="kwses-select2" multiple="multiple" name="<?php echo KWSES()->option_key_name; ?>[meta_keys][]">
			<?php
			foreach ( (array) $meta_keys as $meta_key ) {
				?>
				<option <?php echo $this->wp_es_checked( $meta_key, KWSES()->kwses_settings['meta_keys'], true ); ?> value="<?php echo $meta_key; ?>"><?php echo $meta_key; ?></option>
				<?php
			}
			?>
			</select>
	...
	public function wp_es_taxonomies_settings() {
		 ...
		 */
		$all_taxonomies = apply_filters( 'kwses_tax', get_taxonomies( $tax_args, 'objects' ) );
		if ( is_array( $all_taxonomies ) && ! empty( $all_taxonomies ) ) {
			?>
			<select multiple="multiple" class="kwses-select2" name="<?php echo KWSES()->option_key_name; ?>[taxonomies][]">
			<?php
			foreach ( $all_taxonomies as $tax_name => $tax_obj ) {
				?>
				<option <?php echo $this->wp_es_checked( $tax_name, KWSES()->kwses_settings['taxonomies'], true ); ?> value="<?php echo $tax_name; ?>">
	...
	public function wp_es_author_settings() {
		?>
		<input name="<?php echo KWSES()->option_key_name; ?>[authors]" type="hidden" value="0" />
		<input id="kwses_inlcude_authors" <?php checked( KWSES()->kwses_settings['authors'] ); ?> type="checkbox" value="1" name="<?php echo KWSES()->option_key_name; ?>[authors]" />
		<label for="kwses_inlcude_authors"><?php _e( 'Search in Author display name', 'wp-extended-search' ); ?></label>
	...
	public function wp_es_post_types_settings() {
		 ...
		 */
		$all_post_types = apply_filters( 'kwses_post_types', get_post_types( $post_types_args, 'objects' ) );

		if ( is_array( $all_post_types ) && ! empty( $all_post_types ) ) {
			?>
			<select multiple="multiple" class="kwses-select2" name="<?php echo KWSES()->option_key_name; ?>[post_types][]">
			<?php
			foreach ( $all_post_types as $post_name => $post_obj ) {
				?>
				<option <?php echo $this->wp_es_checked( $post_name, KWSES()->kwses_settings['post_types'], true ); ?> value="<?php echo $post_name; ?>" >
	...
	public function wp_es_terms_relation_type() {
		?>
		<select <?php echo $this->wp_es_disabled( KWSES()->kwses_settings['exact_match'], 'yes' ); ?> id="es_terms_relation" name="<?php echo KWSES()->option_key_name; ?>[terms_relation]">
			<option <?php selected( KWSES()->kwses_settings['terms_relation'], 1 ); ?> value="1"><?php _e( 'AND', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['terms_relation'], 2 ); ?> value="2"><?php _e( 'OR', 'wp-extended-search' ); ?></option>
		</select>
		<p class="description">
			<?php
			if ( 'yes' === KWSES()->kwses_settings['exact_match'] ) {
	...
	public function wp_es_exclude_results() {
		?>
		<input class="regular-text" type="text" value="<?php echo esc_attr( KWSES()->kwses_settings['exclude_date'] ); ?>" name="<?php echo KWSES()->option_key_name; ?>[exclude_date]" id="es_exclude_date" />
	...
	public function wp_es_posts_per_page() {
		?>
		<input min="-1" class="small-text" type="number" value="<?php echo esc_attr( KWSES()->kwses_settings['posts_per_page'] ); ?>" name="<?php echo KWSES()->option_key_name; ?>[posts_per_page]" id="es_posts_per_page" />
	...
	public function wp_es_search_results_order() {
		?>
		<select id="es_search_results_order" name="<?php echo KWSES()->option_key_name; ?>[orderby]">
			<option <?php selected( KWSES()->kwses_settings['orderby'], '' ); ?> value=""><?php _e( 'Relevance', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'date' ); ?> value="date"><?php _e( 'Date', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'modified' ); ?> value="modified"><?php _e( 'Last Modified Date', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'title' ); ?> value="title"><?php _e( 'Post Title', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'name' ); ?> value="name"><?php _e( 'Post Slug', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'type' ); ?> value="type"><?php _e( 'Post Type', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'comment_count' ); ?> value="comment_count"><?php _e( 'Number of Comments', 'wp-extended-search' ); ?></option>
			<option <?php selected( KWSES()->kwses_settings['orderby'], 'rand' ); ?> value="rand"><?php _e( 'Random', 'wp-extended-search' ); ?></option>
		</select>
		<p class="description">
			<?php
		...
		<br />
		<label><input <?php echo $this->wp_es_checked( KWSES()->kwses_settings['order'], array( 'DESC' ) ); ?> type="radio" value="DESC" name="<?php echo KWSES()->option_key_name; ?>[order]" /><?php _e( 'Descending', 'wp-extended-search' ); ?></label>
		<label><input <?php echo $this->wp_es_checked( KWSES()->kwses_settings['order'], array( 'ASC' ) ); ?> type="radio" value="ASC" name="<?php echo KWSES()->option_key_name; ?>[order]" /><?php _e( 'Ascending', 'wp-extended-search' ); ?></label>
	...
	public function wp_es_exact_search() {
		?>
		<label><input <?php echo $this->wp_es_checked( KWSES()->kwses_settings['exact_match'], array( 'yes' ) ); ?> type="radio" value="yes" name="<?php echo KWSES()->option_key_name; ?>[exact_match]" /><?php _e( 'Yes', 'wp-extended-search' ); ?></label>
		<label><input <?php echo $this->wp_es_checked( KWSES()->kwses_settings['exact_match'], array( 'no' ) ); ?> type="radio" value="no" name="<?php echo KWSES()->option_key_name; ?>[exact_match]" /><?php _e( 'No', 'wp-extended-search' ); ?></label>
	...
	public function plugin_links( $links, $file ) {
		if ( KWSES_FILENAME !== $file ) {
			return $links;
		}

		if ( is_array( $links ) ) {
			$links[] = '<a href="https://kwses.secretsofgeeks.com" target="_blank">'
					. __( 'Docs', 'wp-extended-search' )
					. '</a>';
			$links[] = '<a href="https://wordpress.org/plugins/search/kwstech/" target="_blank">'
