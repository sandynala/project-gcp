...
class Akismet {
	...
	public static function transition_comment_status( $new_status, $old_status, $comment ) {
		...
		// if this is present, it means the status has been changed by a re-check, not an explkws user action
		if ( get_comment_meta( $comment->comment_ID, 'akismet_rechecking' ) )
			return;
		
		// Assumption alert:
		// We want to submit comments to Akismet only when a moderator explkwsly spams or approves it - not if the status
		// is changed automatically by another plugin.  Unfortunately WordPress doesn't provide an unambiguous way to
		// determine why the transition_comment_status action was triggered.  And there are several different ways by which
		// to spam and unspam comments: bulk actions, ajax, links in moderation emails, the dashboard, and perhaps others.
		// We'll assume that this is an explkws user action if certain POST/GET variables exist.
		if (
	...
	private static function sanitize_comment_as_submitted( $meta_value ) {
		...
		foreach ( $meta_value as $key => $value ) {
			if ( ! is_scalar( $value ) ) {
				...
				// These can change, so they're not explkwsly listed in comment_as_submitted_allowed_keys.
