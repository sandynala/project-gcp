...
class CSX_Conditions_Manager {
  ...
  public function all_dyno_templates(){
          ...
          WHERE  $wpdb->posts.post_status='publish'"
				...
				$dyno_templates=false;
        foreach ( $templates as $template ) {
					$dyno_templates[] = $template->ID;
				}
				$this->dyno_templates=$dyno_templates;
  }
 ...
 //not to search everytime through templates we'll doit only once. 
  public function cache_conditions(){
    ...
    foreach ($this->dyno_templates as $template_id){
  ...
  public function check_conditions($template_id){
    ...
    foreach($all_conditions as $condition){
     ...
     // print_r($condition);
        ...
        if (!$condition_pass && $name == 'dyno' && $sub_name) $condition_pass = true;
