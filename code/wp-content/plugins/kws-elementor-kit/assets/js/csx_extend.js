var EleCustomSkinSlider = elementorModules.frontend.handlers.Base.extend({
  ...
  /***** start new getSwipperSettings function *******************/
    ...
    getSwiperSettingss: function getSwiperSettingss() {
      ...
      var elementSettings = this.getElementSettings(),
        slidesToShow = +elementSettings[this.getSkinPrefix() + 'slides_to_show'] || 2.5, // this.getElementSettings(this.getSkinPrefix() + 'slides_to_show');
        isSingleSlide = 1 === slidesToShow,
  ...
  /* new way to call the swiper*/    
      ...
      CSXcarousel = this.elements.$carousel;
      CSXswiperSettings = this.getSwiperSettings();
      
      CSX_add_action("swiper", function(){
        CSXswiper = new Swiper(CSXcarousel, CSXswiperSettings);
      });
  ...
  function CSX_reset_masonry(element){
   element.children(".csx-post-dyno").css('margin-top', '');
  }
  ...
  var EleCustomSkinMasonry = elementorModules.frontend.handlers.Base.extend({
    ...
    runMasonry: function runMasonry() {
      ...
      var masonry = new elementorModules.utils.Masonry({
        ...
        items: elements.$postsContainer.children( ".csx-post-dyno" ),//elements.$posts.filter(':visible'),
        columnsCount: this.getSettings('colsCount'),
        verticalSpaceBetween: verticalSpaceBetween
      });
      masonry.run();
      
      CSX_Columns_Count = this.getSettings('colsCount');
      CSX_add_action("ajax", function(){//add reinitilze to ajax
        CSX_reset_masonry(elements.$postsContainer);
        mymasonry = new elementorModules.utils.Masonry({
                container: elements.$postsContainer,
                items: elements.$postsContainer.children( ".csx-post-dyno" ),
                columnsCount: CSX_Columns_Count,
                verticalSpaceBetween: verticalSpaceBetween
              });
              mymasonry.run();
    ...
    onWindowResize: function onWindowResize() {
      this.fitImages();
        CSX_reset_masonry(this.elements.$postsContainer);
      this.runMasonry();
    },
  ...
  function EleCustomSkinItemLink() {
    // jQuery(".csx-link-wrapper").attr("tabindex","0");
    jQuery(".csx-link-wrapper")
      .on("click", function (event) {
        loc = jQuery(this).attr("data-href");
        aTarget = jQuery(this).attr("data-target");
        if (
          !jQuery(event.target).closest(
            ".elementor-swiper-button, .swiper-pagination, .csx-ignore-link"
          ).length
        ) {
      ...
      .on("click", ".csx-ignore-link", function (e) {
  ...
  /*function EleCustomSkinItemLink(){
     // jQuery(".csx-link-wrapper").attr("tabindex","0");
      jQuery(".csx-link-wrapper").click(function(event) {
       loc = jQuery(this).attr("data-href");
       aTarget = jQuery(this).attr("data-target");
       if (!jQuery(event.target).closest('.elementor-swiper-button, .swiper-pagination, .csx-ignore-link').length) {
         if(aTarget == "_blank" || event.ctrlKey || event.metaKey){
              window.open(loc);
          } else {
              window.location = loc;
  ...
  jQuery( document ).ready(function() {
    ...
    CSX_add_action("ajax", function(){EleCustomSkinItemLink()});
  ...
  function CSXcheckInView(){
      jQuery('.csx-lazyload').each(function(){
          if (jQuery(this).inView()){
            id = jQuery(this).attr("data-targetid");
            //console.log(id);
            jQuery(this).removeClass("animation-hidden");
            CSX_load_next_page(id);
          }
      });
  }
