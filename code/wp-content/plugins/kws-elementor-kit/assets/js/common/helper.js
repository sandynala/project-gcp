...
// $(document).on("click", ".ep-pagination .kek-pagination a", function (e) {
//     e.preventDefault();

//     const url = $(this).prop('href');
//     const widgetId = $(this).parents('ul').data('widget-id')
//     var kekWidget = '.elementor-element-' + widgetId;

//     $.ajax({
//         url: url,
//         success: function (response) {
//             var html = $(response).find(kekWidget).html();
//             if (html) $(kekWidget).html(html);
//         }
//     });
// });
        ...
        run: function () {
            ...
            if (this.settings('hide_orphan_tablet') == 'yes') {
                var
                    ...
                    widgetWrapper = $($element).find('.kek-grid-wrapper'),
                    totalItem = widgetWrapper.children().length;

                var result = totalItem % tablet_column;

                if (result !== 0) {
                    $(widgetWrapper).children().slice(-result).addClass('kek-hide-orphan-tablet');
                }
            }
