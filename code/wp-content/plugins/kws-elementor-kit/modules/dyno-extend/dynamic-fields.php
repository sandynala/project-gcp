...
/*-----------------------------------------------------------------------------------*/
function csx_parse_content($t,$post=NULL,$content="",$parent_settings=[]){
	global $wp_query;
  global $csx_index;
  $index = $csx_index;
...
/**  Set custom vars **/
	...
	$post_excerpt = get_the_excerpt() ? get_the_excerpt() : csx_get_my_excerpt($content);
	// add your own custom vars
  $custom_vars=[];
  $custom_vars['_parent_settings']=$parent_settings;
	$custom_vars=apply_filters( 'csx_vars', $custom_vars ); 
...
add_filter( 'csx_dynamic_filter', 'csx_parse_content', 10, 4 );

if (!function_exists("parse_content")) {
  
  function parse_content($t,$post=NULL,$content=""){
     return csx_parse_content($t,$post,$content);
  }

}

function csx_get_my_excerpt($text){
    $raw_excerpt='';
