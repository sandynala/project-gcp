...
class Skin_Posts_XCS extends Skin_Base {
...
//	private $template_cache=[];
  ...
  private function admin_bar_menu(){
    foreach($this->used_templates as $post_id){
      \CSX_Admin_Bar_Menu::add_document($post_id);
    }
  }
    ...
    do_action( 'CSX_after_pagination_controls', $this );
  }
  public function register_navigation_design_controls() {
    do_action( 'CSX_after_style_controls', $this );
  }
    ...
    $this->add_control(
			'skin_template',
			[
				'label' => __( 'Select a default template', 'csx-dyno' ),
        'description' => '<div style="text-align:center;"><a target="_blank" style="text-align: center;font-style: normal;" href="' . esc_url( admin_url( '/edit.php?post_type=elementor_library&tabs_group=theme&elementor_library_type=dyno' ) ) .
                          '" class="elementor-button elementor-button-default elementor-repeater-add">' . 
                          __( 'Create / Edit a Dyno Template', 'csx-dyno' ) . '</a></div>',
    ...
    $this->add_control(
			'use_custom_grid',
			[
				'label' => __( 'Use custom grid?', 'csx-dyno' ),
    ...
    $this->add_control(
        ...
        'description' => '<div style="text-align:center;"><a target="_blank" style="text-align: center;font-style: normal;" href="' . esc_url( admin_url( '/edit.php?post_type=elementor_library&tabs_group=theme&elementor_library_type=custom_grid' ) ) .
                          '" class="elementor-button elementor-button-default elementor-repeater-add">' . 
                          __( 'Create/edit a Custom Grid', 'csx-dyno' ) . '</a></div>',
				'type' => Controls_Manager::SELECT2,
				'label_block' => true,
				'default' => [],
    ...
    do_action( 'CSX_after_control', $this );
				...
				$templates = $wpdb->get_results( 
					"SELECT $wpdb->term_relationships.object_id as ID, $wpdb->posts.post_title as post_title FROM $wpdb->term_relationships
						...
						INNER JOIN $wpdb->terms ON 
							$wpdb->term_taxonomy.term_id=$wpdb->terms.term_id AND $wpdb->terms.slug='dyno'
  ...
  protected function set_custom_grid($grid){// this is for terms we don't need passid so we can actually add them in cache
    ...
    $this->grid = explode('{{csx-article}}',$custom_grid);
  ...
  public function end_grid(){
    ...
    global $xcs_render_dyno, $wp_query,$xcs_index;
    $xcs_index++;
    $old_query=$wp_query;
    ...
    /* move to pro */
    //print_r(ThemeBuilderModule::instance()->get_conditions_manager()->get_documents_for_location( 'dyno' ));
    
    //print_r($this->conditions->get_template());print_r($template);
    
    $template = apply_filters( 'CSX_action_template', $template,$this,$xcs_index );
    $template = $this->get_current_ID($template);
    
    $xcs_render_dyno=get_the_ID().",".$template;
    //echo $xcs_render_dyno;
  ...
  private function get_current_ID($id){
    ...
    $classes = 'elementor-post elementor-grid-item csx-post-dyno';
  ...
  protected function render_post_footer() {
   ...
   ?>
	...
	protected function render_post() {
	  do_action( 'CSX_before_render_post_header', $this );
    $this->render_post_header();
    do_action( 'CSX_after_render_post_header', $this );
    
		if ($this->get_instance_value( 'skin_template' )){
      if ($this->get_instance_value( 'use_keywords' ) == "yes") {
        global $post;
        apply_filters( 'csx_dynamic_filter', "", $post,"",$this->parent->get_settings());//this is for pre-use of custom values
        $template = $this->get_template();
        $new_template = apply_filters( 'csx_dynamic_filter', $template, $post,"",$this->parent->get_settings());
        ...
        __( "Please select a default template! ", 'csx-dyno').'</span></div>';

    do_action( 'CSX_before_render_post_footer', $this );
		$this->render_post_footer();
    do_action( 'CSX_after_render_post_footer', $this );
    ...
    protected function ajax_pagination(){
  	...
  	protected function render_dyno_header() {
      $parent_settings = $this->parent->get_settings();
      $parent_settings[$this->get_id().'_post_slider'] = isset($parent_settings[$this->get_id().'_post_slider'])? $parent_settings[$this->get_id().'_post_slider'] : "";
      ...
      $this->parent->add_render_attribute( 'container', [
        'class' => [
          ...
          'elementor-posts',
           ...
           $this->grid_settings['allow'] ? "csx-custom-grid" : '',
	...
	protected function render_dyno_footer() {
  ...
  protected function render_lazyload_animation() {
    ...
    $animation = \CSX_Loading_Animation::get_lazy_load_animations_html($settings['lazyload_animation']);
    $target= $this->parent->get_id();

		?>
		<nav class="csx-lazyload elementor-pagination" data-targetid="<?php echo $target; ?>">
            <?php echo $animation; ?>
  ...
  protected function render_loadmore_button() {
    ...
    $data=htmlentities(json_encode($args), ENT_QUOTES);
		...
		?>
		<nav class="elementor-button-wrapper elementor-pagination csx-load-more-button" data-settings="<?php echo $data; ?>">
  ...
  private function slider_elements(){
    ...
    do_action( 'CSX_after_slider_elements', $this );
  }
  
  private function nothing_found(){
      $this->render_dyno_header();
      ...
      do_action( 'CSX_not_found', $this );
      echo  $message;
      $this->render_dyno_footer();
  }
  ...
  public function render() {
    ...
    do_action("CSX_before_dyno_query",$query,$this);
    
    // start de incercat de bagat in action CSX_before_dyno_query
    if (class_exists('CSX_Conditions_Manager')) 
      if($this->get_instance_value( 'is_display_conditions' )) $this->conditions = new \CSX_Conditions_Manager();
    ...
    if ( ! $query->found_posts ) {
       $this->nothing_found();
		...
		// It's the global `wp_query` it self. and the dyno was started from the theme.
		if ( $query->in_the_dyno ) {
    ...
    do_action("CSX_after_dyno_query",$query,$this);
    ...
    wp_reset_postdata();
      ...
      //let's position ourselfs inside the dyno item
      global $wp_query;
      $old_query=$wp_query;
      $new_query=new \WP_Query( array( 'p' => get_the_ID(), 'post_type' => get_post_type() ) );
      $wp_query = $new_query;
      ...
      $wp_query = $old_query;//get out of dyno item
      if ( $setting_key ) {
...
class Skin_Archive_XCS extends Skin_Posts_XCS {
