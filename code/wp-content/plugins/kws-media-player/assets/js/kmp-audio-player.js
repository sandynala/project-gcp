(function ($) {
	...
	var WidgetAudioPlayerHandler = function($scope, $) {
		var container = $scope.find('.cf-audio-player-wrapper');
		...
		function initAudio(elem,id) {
			...
			if(style=='style-c'){
				$('.'+id+' .cf-player .trackimage img').attr('src',thumb);
			}			
			if(style=='style-e'){
				$('.'+id+' .cf-player .ap-st5-img').css('background','url('+ thumb +')').css('transition', 'background 0.5s linear');				
			}
			if(style=='style-extend'){
				$('.'+id+' .cf-player-bg-img').css('background','url('+ thumb +')');
				$('.'+id+' .cf-player .trackimage img').attr('src',thumb);
			}
			...
			$('.'+id+' .cf-player .title').text(title);
			$('.'+id+' .cf-player .artist').text(artist);
		...
		function stopAllAudio(){
			...
			$('.cf-audio-player-wrapper .play').removeClass('hidden');
			$('.cf-audio-player-wrapper .pause').removeClass('visible');
		}
		...
		function skipAudio(id,diff) {
			//song.pause();
			song.currentTime += diff;
			UpdateSeek(id);
		}
		...
		$('.'+id+' .pause').on('click',function(e) {
			...
			// skip 10 sec
		$('.'+id+' .skipfwd').on('click',function(e) {
			e.preventDefault();
			skipAudio(id,10);
		});
		$('.'+id+' .skiprew').on('click',function(e) {
			e.preventDefault();
			skipAudio(id,-10);
		});
		...
		$('.'+id+' .vol-icon-toggle').on('click',function(e) {
			e.preventDefault();
			$('.'+id+' .cf-volume-bg').toggleClass('show');
		});
	...
	$(window).on('elementor/frontend/init', function () {
		elementorFrontend.hooks.addAction('frontend/element_ready/cf-audio-player.default', WidgetAudioPlayerHandler);
	});
})(jQuery);
...
function skipAudio(id,song,diff) {
	//song.pause();
	song.currentTime += diff;
	UpdateSeek(id);
}
