...
class KWSES_Settings_CPT {
	...
	public function __construct() {
		...
		add_filter( 'bulk_actions-edit-kwses_setting', array( $this, 'remove_bulk_actions' ) );
	}
	...
	public function register_setting_posttype() {
		// CPT labels.
		$labels = array(
			'name'               => 'KWSES ' . __( 'Setting Names', 'wp-extended-search' ),
			'singular_name'      => 'KWSES ' . __( 'Setting Name', 'wp-extended-search' ),
		...
		register_post_type( 'kwses_setting', $args );

		// Register function to handle delete post action.
		add_action( 'delete_post', array( $this, 'delete_kwses_setting' ) );
	}
	...
	public function post_submit_box_js() {
		if ( get_current_screen()->id !== 'kwses_setting' ) {
			return; // Return if not the kwses setting CPT.
		} ?>
	...
	public function remove_quick_edit( $actions, $post ) {
		if ( get_post_type( $post ) === 'kwses_setting' ) {
	...
	/**
	 * Delete KWSES setting when post is deleted from trash.
	 *
	 * @since 2.0
	 * @param int $post_id Post ID.
	 */
	public function delete_kwses_setting( $post_id ) {
		if ( get_post_type( $post_id ) === 'kwses_setting' ) {
	...
	public function register_meta_boxes() {
		add_meta_box( 'kwses-cpt-configure-setting', 'Search', array( $this, 'primary_meta_box' ), 'kwses_setting', 'side' );

		// Only display the info metabox when post is published.
		if ( ! empty( $_GET['post'] ) && $this->is_setting_published( intval( $_GET['post'] ) ) ) {
			add_meta_box( 'kwses-cpt-info', 'Uses:', array( $this, 'setting_info' ), 'kwses_setting', 'normal' );
		}
	}
	...
	public function primary_meta_box( $post ) {
		if ( $this->is_setting_published( $post ) ) {
			?>
			<a class="button button-primary button-large" href="<?php echo admin_url( 'admin.php?page=wp-es&kwsessid=' . $post->ID ); ?>"><?php _e( 'Configure Search Setting', 'wp-extended-search' ); ?></a>
			<?php
		} else {
	...
	public function setting_info( $post ) {
		$using_template = locate_template( 'kwses-searchform-' . $post->ID . '.php' );
		...
		<table class="form-table">
			...
			<td>
				...
				printf( __( 'Go to <a href="%1$s">Appearance &raquo; Widgets</a> and add <em>%2$s Search Form</em> widget.', 'wp-extended-search' ), admin_url( 'widgets.php' ), 'KWSES' );
				?>
			</td>
			</tr>
			<tr>
			...
			<td>
				<input title="<?php _e( 'Click to copy.', 'wp-extended-search' ); ?>" class="kwses-display-input" readonly="readonly" type="text" value="[kwses_search_form kwsessid='<?php echo $post->ID; ?>']" />
			...
			<td>
				<textarea title="<?php _e( 'Click to copy.', 'wp-extended-search' ); ?>" rows="5" class="kwses-display-input" readonly="readonly">
if ( function_exists( 'kwses_search_form' ) ) {
	kwses_search_form( array( 
		'kwsessid' => <?php echo $post->ID; ?> 
	) );
}				</textarea>
			...
			<td>
				<input title="<?php _e( 'Click to copy.', 'wp-extended-search' ); ?>" class="kwses-display-input" readonly="readonly" type="text" value="&#x3C;input type=&#x27;hidden&#x27; value=&#x27;<?php echo $post->ID; ?>&#x27; name=&#x27;kwsessid&#x27; /&#x3E;" />
		...
		<hr />
		<dl class="kwses-params">
			<?php
			foreach ( KWSES()->kwses_search_form->get_form_default_args() as $key => $key_desc ) {
				?>
				<dt><?php echo $key; ?></dt>
				<dd>
					<p class="kwses-param-desc"><?php echo $key_desc; ?></p>
					<p class="kwses-param-default"><em><?php echo __( 'Default value:', 'wp-extended-search' ) . ' ' . KWSES()->kwses_search_form->get_form_default_args( $key ); ?></em></p>
				</dd>
				<?php
			}
			?>
		...
		<hr />
		<div class="kwses-custom-templates-help">
			...
			<ol>
				<li>
					<p class="description">
						...
						printf( __( 'Create <code>%1$s</code> in your child theme, where %2$d is the setting ID.', 'wp-extended-search' ), "kwses-searchform-{$post->ID}.php", $post->ID );
						?>
					</p>
				</li>
				<li>
					<p class="description">
						...
						printf( __( 'Add custom search form code to it. If you are not sure how to write custom form code then copy the content from the theme\'s <code>%1$s</code> and add to <code>%2$s</code> with your modifications.', 'wp-extended-search' ), 'searchform.php', "kwses-searchform-{$post->ID}.php" );
						?>
					</p>
				</li>
				<li><p class="description"><?php _e( 'Then use the KWSES Search Widget, Shortcode, or PHP function to display the search form.', 'wp-extended-search' ); ?></p></li>
			</ol>
			...
			<ul class="ul-disc">
				...
				<li>
					<p class="description">
						...
						printf( __( 'Do not add <code>%s</code> hidden field to the custom template, it will be added by the plugin.', 'wp-extended-search' ), 'kwsessid' );
						?>
					</p>
				</li>
			</ul>
