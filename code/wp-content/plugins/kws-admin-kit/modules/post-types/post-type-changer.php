 ...
 * Post Type Changer
 *
 * Allow changing of a post type while editing a post
 *
...
/**
 * The main post type changer class
 *
 * @since 1.0.0
 */
final class Post_Type_Changer {
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'post-type-changer' );
	}
	...
	public function admin_init() {
		...
		// Override
		add_action( 'wp_ajax_post_type_changer',  array( $this, 'handle_ajax' ) );
		...
		do_action( 'post_type_changer', $this );
	}
	...
	public function metabox() {
		...
		?><div class="misc-pub-section misc-pub-section-last post-type-changer">
			<label for="kws_post_type"><?php esc_html_e( 'Article Type:', 'post-type-changer' ); ?></label>
			...
			<?php if ( current_user_can( $cpt_object->cap->publish_posts ) ) : ?>
				...
				<a href="#" id="edit-post-type-changer" class="hide-if-no-js"><?php esc_html_e( 'Edit', 'post-type-changer' ); ?></a>
				<div id="post-type-select">
					<select name="kws_post_type" id="kws_post_type"><?php
					...
					?></select>
					<a href="#" id="save-post-type-changer" class="hide-if-no-js button"><?php esc_html_e( 'OK', 'post-type-changer' ); ?></a>
					<a href="#" id="cancel-post-type-changer" class="hide-if-no-js"><?php esc_html_e( 'Cancel', 'post-type-changer' ); ?></a>
				</div><?php

				wp_nonce_field( 'post-type-selector', 'kws-nonce-select' );
	...
	public function add_column( $columns = array() ) {
		return array_merge( $columns, array( 'post_type' => esc_html__( 'Type', 'post-type-changer' ) ) );
	}
	...
	public function quick_edit( $column_name = '' ) {
		...
		<div id="kws_quick_edit" class="inline-edit-group wp-clearfix">
			<label class="alignleft">
				<span class="title"><?php esc_html_e( 'Post Type', 'post-type-changer' ); ?></span><?php
	...
	public function quick_edit_bulk( $column_name = '' ) {
		...
		<label id="kws_bulk_edit" class="alignleft">
			<span class="title"><?php esc_html_e( 'Post Type', 'post-type-changer' ); ?></span><?php
	...
	public function quick_edit_script( $hook = '' ) {
		...
		wp_enqueue_script( 'kws_quickedit', KWSADM_ADM_ASSETS_URL . 'js/quickedit.js', array( 'jquery' ), $this->asset_version, true );
	}
	...
	public function block_editor_assets() {
		...
		$change_url = add_query_arg(
			array(
				'action'           => 'post_type_changer',
				'kws-nonce-select' => wp_create_nonce( 'post-type-selector' ),
				'post_id'          => get_the_ID(),
			),
		...
		wp_enqueue_script(
			'kws_blockeditor',
			KWSADM_ADM_ASSETS_URL . 'js/block.js',
	...
	public function select_box( $bulk = false ) {
		...
		?><select name="kws_post_type" id="kws_post_type"><?php
			...
			if ( true === $bulk ) :
				?><option value="-1"><?php esc_html_e( '&mdash; No Change &mdash;', 'post-type-changer' ); ?></option><?php
			endif;
	...
	public function handle_ajax() {
		...
		if (
			   empty( $_GET['kws_post_type'] )
			|| empty( $_GET['kws-nonce-select'] )
			|| empty( $_GET['post_id'] )
		) {
			return wp_die( esc_html__( 'Missing data.', 'post-type-changer' ) );
		}

		// Post type information
		$post_id          = absint( $_GET['post_id'] );
		$post_type        = sanitize_key( $_GET['kws_post_type'] );
		$post_type_object = get_post_type_object( $post_type );

		// Bail if user isn't capable or nonce fails
		if ( ! current_user_can( $post_type_object->cap->publish_posts )
			|| ! wp_verify_nonce( $_GET['kws-nonce-select'], 'post-type-selector' ) ) {
			return wp_die( esc_html__( 'Sorry, you cannot do this.', 'post-type-changer' ) );
		}
	 ...
	 * post type when the user explkwsly intends to.
	 *
	 * - Not during autosave
	 * - Check nonce
	...
	public function override_type( $data = array(), $postarr = array() ) {
		...
		if ( empty( $_REQUEST['kws_post_type'] ) || empty( $_REQUEST['kws-nonce-select'] ) ) {
			return $data;
		}
		...
		$post_type        = sanitize_key( $_REQUEST['kws_post_type'] );
		$post_type_object = get_post_type_object( $post_type );
		...
		// See: https://github.com/JJJ/post-type-changer/issues/9
		...
		if ( ! wp_verify_nonce( $_REQUEST['kws-nonce-select'], 'post-type-selector' ) ) {
			return $data;
		}
	...
	?>
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				...
				jQuery( '#edit-post-type-changer' ).on( 'click', function(e) {
					jQuery( this ).hide();
					jQuery( '#post-type-select' ).slideDown();
					e.preventDefault();
				});
				jQuery( '#save-post-type-changer' ).on( 'click', function(e) {
					jQuery( '#post-type-select' ).slideUp();
					jQuery( '#edit-post-type-changer' ).show();
					jQuery( '#post-type-display' ).text( jQuery( '#kws_post_type :selected' ).text() );
					e.preventDefault();
				});
				jQuery( '#cancel-post-type-changer' ).on( 'click', function(e) {
					jQuery( '#post-type-select' ).slideUp();
					jQuery( '#edit-post-type-changer' ).show();
					e.preventDefault();
				});
		...
		<style type="text/css">
			...
			#post-type-select select#kws_post_type {
				margin-right: 2px;
			}
			#post-type-select a#save-post-type-changer {
			...
			#post-body .post-type-changer::before {
				content: '\f109';
	...
	/**
	 * Whether or not the current file requires the post type changer
	 *
	 * @since 1.1.0
	 *
		...
		if (
			...
			(
				...
				(
					...
					&&
					(
						in_array( $_REQUEST['action'], array( 'inline-save', 'post_type_changer' ), true )
					)
				)
			)
		) {
			...
			// Allowed admin pages
			$pages = apply_filters( 'kws_allowed_pages', array(
				'post.php',
				'edit.php',
				'admin-ajax.php' // Block Editor
			) );

			// Only show changer when editing
	...
	private function get_post_type_args() {
		return (array) apply_filters( 'kws_post_type_filter', array(
			'public'  => true,
			'show_ui' => true
		) );
	}
